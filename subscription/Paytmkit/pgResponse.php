...
if($isValidChecksum == "TRUE") {
	...
	{ 
			...
			if($_POST['STATUS']=="TXN_SUCCESS" || $_POST['TXNAMOUNT']==0)
		{
			...
			$exp=explode("SU",$ORDERID);
			$exp1=$exp[1];
			if($exp1==3)
			{$oneYearOn = date('Y-m-d', strtotime(' + 90 days'));}
			if($exp1==6)
			{$oneYearOn = date('Y-m-d', strtotime(' + 182 days'));}
			if($exp1==12)
			{$oneYearOn = date('Y-m-d', strtotime(' + 365 days'));}
			...
			if(isset($_POST['STATUS']))
			{
				...
				while($row=mysql_fetch_array($result))
				{
				 ...
				 $result2=mysql_query("SELECT *FROM subscription");
				...
				$sql="INSERT INTO subscription VALUES('$idsm','$email','$Address','$TXNAMOUNT','$TXNDATE','$CURRENCY','$ORDERID','$TXNID','$PAYMENTMODE','$MID','$STATUS','$RESPCODE','$RESPMSG','$GATEWAYNAME','$BANKTXNID','$BANKNAME','$expdate')";
				$sql2="UPDATE user SET pro='yes' WHERE email='$email'";
				if (mysql_query($sql)) 
				{
					if (mysql_query($sql2)) 
					{
