...
if(!isset($_GET['view']))
{
	echo"<input type='button' onclick=\"window.location.href='certificates-delivery.php'\" value='view Uncompleted orders'>";
	...
	$query5=mysql_query("SELECT *FROM delivery WHERE completed='yes'");
	while($row50=mysql_fetch_array($query5))
	{
		...
		echo"
		<tr><td><form id='myForm' action='./ac.php/?view=1' method='post'>
	...
	if(!isset($_GET['completed']))
{
	...
	<input type='button' onclick='window.location.href=\"./ac.php/?view=1&completed=no&iv=".$ng."\"' value='Mark Order as Uncompleted'>
	</div>";
}	
}
if(isset($_GET['completed']))
{
	...
	if($r=='no')
	{
		if(mysql_query("UPDATE delivery SET completed='no' WHERE ORDERID='$fs'"))
		{
			...
			Delivery sucessfully marked as uncompleted
			<a style='color:red' href='./ac.php/?view=1&completed=no&iv=".$fs."'>To Reset Click here</a>
			</h1>";
		}
		else
		{
