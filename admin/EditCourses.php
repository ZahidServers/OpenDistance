...
if($numrows>0)
{
	...
	while($rowsz=mysql_fetch_assoc($query))
	{
		...
		$hrs=$rowsz['hrs'];
		$src="$iv.png";
		echo "<div style='vertical-align:top;' class='box'><h3><a href='#'onclick=\"window.open('./ec.php?iv=".$iv."','Edit Course','height:800px,width:900px')\"><div style='float:left;'><img  width='210px;' height='120px;' border='4' src='../course/iv/$src' alt='$title'></img></div><div>$title</a><h3><p style='font-size:15px;'>$description</p><p style='font-size:12px;'>Type:$type <b>|</b> Sub-Type:$subtype <b>|</b> Duration:$hrs</p></div><br></div><br>";
	}
}
else{
...
if(!isset($_GET['k']))
{
	...
	while($rowsz=mysql_fetch_array($query5))
	{
		...
		$src="$iv.png";
		echo "<div class='box'>
		<h3><a href='#' onclick=\"window.open('./ec.php?iv=".$iv."','Edit Course','height:800px,width:900px,left=10,top=10,scrollbars=yes,menubar=no')\"><img width='210px;' height='120px;' border='4' src='../course/iv/$src' alt='$title'></img><br>$title</a>&nbsp;&nbsp;&nbsp;<h3>
