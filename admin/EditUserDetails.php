 ...
 while($rowf=mysql_fetch_array($resultf))
{
	...
	$user=$rowf['name'];
	$idfdrs++;
	echo"<tr><th><a href='./EditUserDetails.php?q=$q&delete=1&id=$idfdr'>Edit</a></th><th>$idfdrs</th><td>$user</td><td>$email</td><td>$gender</td><td>$phone</td><td>$dob</td><td>$father</td><td>$mother</td>
...
function confirmDelete(delUrl) {
  if (confirm("Are you sure you want to change this user details")) {
   document.location.href = "'.$utcf.$uzee.'";
	...
	{
		$idsmz=$_GET['id'];
		echo"<script>window.open('./eud.php?id=$idsmz','Edit User Details','width=900,height=650');</script>";
	}
}
}
?>
